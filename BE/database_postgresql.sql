...
CREATE INDEX idx_refresh_token_expiry ON refresh_token(expiry_time);
...
CREATE TABLE jobs (
    ...
    deadline DATE,
    status VARCHAR(20) DEFAULT 'open' CHECK (status IN ('draft', 'open', 'closed', 'expired', 'rejected')),
    created_by UUID,
    updated_by UUID,
...
CREATE TABLE applications (
    ...
    cv_url VARCHAR(255),
    cover_letter TEXT,
    status VARCHAR(20) DEFAULT 'pending' CHECK (status IN ('pending', 'reviewing', 'approved', 'rejected', 'withdrawn')),
